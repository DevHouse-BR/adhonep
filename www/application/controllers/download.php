...
class Download extends CI_Controller {
	...
	function salvaDownload(){
		...
		if(array_key_exists('caminho', $_FILES)){
			if(strlen($_FILES['caminho']["tmp_name"])>0){
				...
				if (!$this->upload->do_upload('caminho')){
					echo('{success: false, errors: {caminhoField: "' . $this->upload->display_errors() . '"}, errormsg:"' . $this->upload->display_errors() . '"}');
					return;
				}	
				else{
		...
		if($this->MDownloads->salvaDownload($data)) echo("{success:true}");
		else echo("{success:false, errormsg:'Não foi possível gravar informações no banco de dados.'}");
	}
	...
	function removeDownloads(){
		...
		foreach($lista as $download){
			if($this->MDownloads->removeDownloads(array($download['iddownloads']))){
