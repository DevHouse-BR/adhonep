...
<form action="<?php echo JRoute::_('index.php?option=com_user#content'); ?>" method="post" id="josForm" name="josForm" class="form-validate user">
	...
	<fieldset>
		...
		<div class="name">
			...
			<div class="input-field-l"><input type="text" name="name" id="name" value="<?php echo $this->escape($this->user->get('name')); ?>" class="inputbox validate required none namemsg" maxlength="50" /></div>
		</div>
		<div class="user">
			<label id="usernamemsg" for="username"><?php echo JText::_('Username'); ?>: *</label>
			<div class="input-field-l"><input type="text" id="username" name="username"  value="<?php echo $this->escape($this->user->get('username')); ?>" class="inputbox validate required username usernamemsg" maxlength="25" /></div>
		</div>
		<div class="email">
			<label id="emailmsg" for="email"><?php echo JText::_('Email'); ?>: *</label>
			<div class="input-field-l"><input type="text" id="email" name="email"  value="<?php echo $this->escape($this->user->get('email')); ?>" class="inputbox validate required email emailmsg" maxlength="100" /></div>
		</div>
	</fieldset>
	<fieldset>
		<div class="pass">
			<label id="pwmsg" for="password"><?php echo JText::_('Password'); ?>: *</label>
			<div class="input-field-l"><input type="password" id="password" name="password" value="" class="inputbox required validate-password" /></div>
		</div>
		<div class="verify_pass">
			<label id="pw2msg" for="password2"><?php echo JText::_('Verify Password'); ?>: *</label>
			<div class="input-field-l"><input type="password" id="password2" name="password2" value="" class="inputbox required validate-passverify" /></div>
		</div>
	</fieldset>
